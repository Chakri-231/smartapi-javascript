...
const totp = '684523';    
const MAX_SYMBOLS = 3; // Monitors the top 3 symbols for BOTH Gainers and Losers categories.
...
// Polling intervals
const SYMBOL_LIST_POLL_INTERVAL_MS = 1 * 60 * 1000; // 3 minutes: List refresh interval
...
function checkCandleBodyAndAlert(symbol, exchange, openPrice, ltp) {
    ...
    if (isNaN(parsedOpen) || parsedOpen <= 0) {
        // Log a warning if open price is invalid, but continue
        return;
    }
    ...
    if (Math.abs(percentDiff) >= CANDLE_THICKNESS_PERCENT_THRESHOLD) {
        ...
        // Candle body is too thin to trigger an alert
    }
}
 ...
 * @param {string} dataType 'PercPriceGainers' or 'PercPriceLosers'
 * @returns {Map} A map of tokens to symbol data for the fetched symbols.
 */
async function fetchAndProcessSymbols(dataType) {
    ...
    try {
        ...
        if (!response.status || !response.data || response.data.length === 0) {
            console.log(`No Top ${categoryName} data available in API response.`);
            return tempCache;
        }
        ...
        for (const symbol of topSymbols) {
            // 1. Store NFO Token
            tempCache.set(symbol.symbolToken, {
                exchange: 'NFO', 
            ...
            const nseScrip = await findNSEEquityToken(symbol.tradingSymbol);
            if (nseScrip) {
                tempCache.set(nseScrip.token, {
                    ...nseScrip,
                    category: categoryName
                });
            }
...
/**
 * Fetches Top N Price Gainers and Losers, finds their NSE equivalents, and updates the global cache.
 */
async function pollTopGainersAndLosers() {
    console.log(`\n[${new Date().toLocaleTimeString()}] --- 🥇 POLLING: Refreshing Top ${MAX_SYMBOLS} Gainers & Losers List ---`);
    if (!smart_api) return;

    // Initialize an empty map for the new set of tokens
    ...
    tokenCache = newCache;
    console.log(`✅ CACHE UPDATED: ${tokenCache.size} total tokens (NFO + NSE, Gainers & Losers) stored.`);
}
...
async function pollMarketDataAndAlerts() {
    if (tokenCache.size === 0) {
        console.log(`[${new Date().toLocaleTimeString()}] --- 🕯️ IDLE: Waiting for Top symbols list to be populated...`);
        return;
    }

    console.log(`\n[${new Date().toLocaleTimeString()}] --- 📊 POLLING: Fetching live data for ${tokenCache.size} symbols and checking 1-minute candle body...`);
...
async function executeSmartAPIActions() {
    ...
    console.log(`> Top Symbol List (Gainers & Losers): Refreshed every ${SYMBOL_LIST_POLL_INTERVAL_MS / 60000} minutes.`);
    console.log(`> Live Data/Alert Check: Refreshed every ${MARKET_DATA_POLL_INTERVAL_MS / 60000} minute (Thick Candle Check).`);

    // Start 3-minute poll for gainers/losers list (which also finds NSE tokens)
